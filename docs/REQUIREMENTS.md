...
`repeater-nms` 用于 10G IPVB 中继器 RX10 的网管开发，第一版目标覆盖设备管理、SNMP GET 采集、SNMP Trap 实时接收展示、告警管理、基础 Web 页面和服务器部署。
...
- collector 必须能容忍单个 Trap 或单个 OID 失败，不得整体崩溃。
- 所有敏感配置通过环境变量注入。
